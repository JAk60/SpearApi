...
function isQuestionData(data: WSMessage): data is QuestionData {
	return (
		typeof (data as QuestionData).question === "string" &&
...
export default function SpearForm() {
	...
	>([]);
	const [refinedClassifications, setRefinedClassifications] = useState<Record<
		string,
		Record<string, number>
	> | null>(null);
	const [highClassifications, setHighClassifications] = useState<Record<
		string,
		Record<string, number>
	> | null>(null);
		...
		wsRef.current.onmessage = (event: MessageEvent) => {
			...
			if (isCompletionData(data)) {
				...
				console.log(
					...
					data.refined_classifications
				);
				console.log(
					"high_confidence_classifications:",
					data.high_confidence_classifications
				);
				toast({
					title: "Processing Complete",
					description: `Analysis completed in ${data.timing_summary.total_time.toFixed(
						2
					)} seconds`,
				});
				wsRef.current?.close();
				...
				toast({
					...
					title: "Error",
					description:
						"error" in data ? data.error : "Unknown error occurred",
				});
				setIsProcessing(false);
			}
		};
			...
			{questionHistory.length > 0 && (
				...
				{refinedClassifications && (
					<Card className="p-6 w-1/2">
						...
						<div className="space-y-4">
							{Object.entries(refinedClassifications).map(
				...
				{highClassifications && (
					<Card className="p-6">
						...
						<div className="space-y-4">
							{Object.entries(highClassifications).map(
								([layer, classifications]) => (
									<div key={layer} className="border-b pb-4">
										<h4 className="font-medium mb-2">
											{layer}
