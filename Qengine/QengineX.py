...
def generate_layer_specific_question(layer_type: LayerType, top_class: str, second_class: str,
    ...
    base_prompts = {
        LayerType.CATEGORY: f"""Generate a yes/no question to confirm if this is a {top_class} task:
            Focus on distinguishing from {second_class}.
            Example: "Is this task primarily focused on {top_class.lower()} activities?"
            Respond with a single yes/no question.""",

        LayerType.MISSION_TYPE: f"""Generate a yes/no question to confirm if this is a {top_class} mission:
            Focus on distinguishing from {second_class}.
            Example: "Is this operation specifically designated as a {top_class.lower()} mission?"
            Respond with a single yes/no question.""",

        LayerType.CRITICALITY: f"""Generate a yes/no question to confirm {top_class} criticality:
            Focus on distinguishing from {second_class}.
            Example: "Does this task require immediate attention within 24 hours?"
            Respond with a single yes/no question.""",

        LayerType.LEVEL: f"""Generate a yes/no question to confirm if this operates at {top_class} level:
            Focus on distinguishing from {second_class}.
            Example: "Is this operation conducted at the {top_class.lower()} level?"
            Respond with a single yes/no question.""",

        LayerType.ACTION: f"""Generate a yes/no question to confirm if {top_class} is required:
            Focus on distinguishing from {second_class}.
            Example: "Does this task specifically require {top_class.lower()} action?"
            Respond with a single yes/no question.""",

        LayerType.ENTITY: f"""Generate a yes/no question to confirm if this is a {top_class}:
            Focus on distinguishing from {second_class}.
            Example: "Is this entity classified as a {top_class.lower()}?"
            Respond with a single yes/no question.""",

        LayerType.FROM: f"""Generate a yes/no question to confirm if this is from {top_class}:
            Focus on distinguishing from {second_class}.
            Example: "Does this originate directly from {top_class.lower()}?"
            Respond with a single yes/no question.""",

        LayerType.TASK_OBJECTIVE: f"""Generate a yes/no question to confirm if this is {top_class}:
            Focus on distinguishing from {second_class}.
            Example: "Is {top_class.lower()} the primary objective?"
            Respond with a single yes/no question.""",

        LayerType.CONSTRAINTS: f"""Generate a yes/no question to confirm if {top_class} is the main constraint:
            Focus on distinguishing from {second_class}.
            Example: "Is {top_class.lower()} the primary limiting factor?"
            Respond with a single yes/no question.""",

        LayerType.OBJECTIVE: f"""Generate a yes/no question to confirm if {top_class} is the main objective:
            Focus on distinguishing from {second_class}.
            Example: "Is {top_class.lower()} the primary goal?"
            Respond with a single yes/no question."""
    }
...
def process_naval_classification(model_output: Dict[str, Dict[str, float]],
                                 confidence_threshold: float = 0.20) -> None:
    """Process naval classification results with multiple rows for each question pair."""
        ...
        if key in multi_label_layers:
            ...
            if len(probable_labels) > 1:
                ...
                layer_results.append({
                    'layer_number': idx + 1,
                    'layer_type': layer_type.name,
                    'class_1': probable_labels[0][0],
                    'class_2': None,
                    'prob_1': probable_labels[0][1],
                    'prob_2': 0,
                    'prob_diff': 0,
            ...
            layer_results.append({
                ...
                'prob_2': second_prob,
                'prob_diff': prob_diff,
                'question': generate_layer_specific_question(layer_type, top_class, second_class, prob_diff)
                if prob_diff < confidence_threshold else "Classification confident"
            })
    print("<---CSV save--->")
    df = pd.DataFrame(layer_results)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    df.to_csv(f'../Qengine/Qlogs/naval_classification_results_{timestamp}.csv', index=False)
# Rest of the functions (save_to_csv, generate_layer_specific_question, determine_layer_type)
